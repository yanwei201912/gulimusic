...
可拖拽的导航:
    ...
    滑屏元素:
        ul:
            ...
            设置ul的white-space为no-wrap 强制让ul内的行内块元素不换行
...
;(function(){
    ...
    var szStartX=0;//手指初始位置
    var minX=nav.clientWidth-list.offsetWidth;

    //手动橡皮筋效果
    var lastPoint=0;//手中上一次的位置
    ...
    nav.addEventListener("touchstart", function(ev){
        ...
        var touchC =ev.changedTouches[0];//获取第一根手指
        eleStartX=transform.css(list,"translateX");
        szStartX = touchC.clientX; //获取手指点击时在X轴上距离视口的距离
    ...
    nav.addEventListener("touchmove", function (ev) {
        ...
        var szDisX=szNowX-szStartX;//获取手指滑动的距离
        var translateX=eleStartX+szDisX;

        //手动橡皮筋效果
        var nowPoint=touchC.clientX;//手指当前的位置
        var nowTime=new Date().getTime();//获取手指滑动时的实时时间
        disTime=nowTime-lastTime;//获取手指滑动时间
    ...
    function fast(disPoint,disTime,list){
        //获取每毫秒的速度
         ...
         if (targetX > 0) {
             targetX = 0;
             bezier = "cubic-bezier(.09,1.51,.65,1.73)";
         } else if (targetX < minX) {
             targetX = minX;
             bezier = "cubic-bezier(.09,1.51,.65,1.73)";
         }
         list.style.transition=time+"s"+bezier+"transform";
